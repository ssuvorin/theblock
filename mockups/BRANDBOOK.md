...
A career brain that remembers every relationship across email, meetings and
...
The answer is ranked by public opportunity first, then by evidence-backed warm path.
Each card uses a 1px `--bd` border on `--lowest` and shows role, organization,
location, verification status, `checked_at` and the public URL. `verified_open_role`,
`hiring_signal`, `unverified` and `stale` differ by text and border treatment, never
colour alone. Only `verified_open_role` receives the accent verification border;
`--pos` is reserved for opportunity signals. Public citations and private relationship
citations are separate labelled blocks. A card without relationship evidence remains
visible and contains the dashed message `NO WARM PATH FOUND`. Each card region has at
most one filled-orange action.
...
|---|---|---|
| **Convex** | Derived semantic index for owner-scoped relationship evidence. PostgreSQL remains the canonical system of record. | `SEMANTIC INDEX · N CHUNKS` |
| **Context.dev** | Searches current public opportunities and enriches resolved organizations. Private relationship text is never sent to it. | `MARKET SEARCH · N SOURCES CHECKED` |
| **Collabute** | Read-only import of existing meetings, summaries, decisions and action items. The CRM does not schedule or record calls. | `MEETING IMPORT · N MEETINGS` |
...
judge-facing *Built on* frame. Every `N` binds to a live counter; unavailable counters
render `—` and are never replaced with fabricated metrics.
...
| **Daniel Ruiz** | Ops lead, Palm Logistics, Dubai | WhatsApp contact, role appeared at his company |
